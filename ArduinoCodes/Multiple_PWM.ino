...
void forward(const std_msgs::Float32MultiArray& joyinputx){ // joyinput is positive
     enableforward();
     writePWM(joyinputx.data);
}

void reverse(const std_msgs::Float32MultiArray& joyinputx){ // joyinput is negative
     enablereverse();
     writePWM(joyinputx.data);
}

void turnRight(const std_msgs::Float32MultiArray& joyinputy){ // joyinput is negative
     writePWM(joyinputy.data);
}

void turnLeft(const std_msgs::Float32MultiArray& joyinputy){ // joyinput is positive
     writePWM(joyinputy.data);
}
void writePWM(float joyInpArray[]){
      analogWrite(r1pwm, abs(joyInpArray[0] * 255));
      analogWrite(l1pwm, abs(joyInpArray[1] * 255));
      analogWrite(r2pwm, abs(joyInpArray[2] * 255));
      analogWrite(l2pwm, abs(joyInpArray[3] * 255));
      analogWrite(r3pwm, abs(joyInpArray[4] * 255));
      analogWrite(l3pwm, abs(joyInpArray[5] * 255));
}
void halt(int pwm){
    for(int i=22; i<=27; i++){
        digitalWrite(i, LOW);
