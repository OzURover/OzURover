...
class Driver:
	...
	def new_target(self, dist, deg, now=False):
		if now:
			self.curr_index = 0
			self.targets = [{
				"x": dist,
		...
		if self.is_waiting():
			...
			if self.zero["reset"]:
				...
				if not self.is_target_angle_reached(DEG):
					...
					m = 0.375 if abs(self.targets[self.curr_index]
					                 ["x"] - (self.current["pos"])) < 30 else 1
				...
				sys.stdout.write("travelled: " + str(self.current["pos"]) + " DEG: " + str(DEG) + " target: " + str(
					self.targets[self.curr_index]["deg"]) + " target_angle_diff: " + str(self.target_angle_diff(DEG)[0]) + " pwm: " + str(pwm) + "   \r")
				sys.stdout.flush()
...
if __name__ == "__main__":
	...
	DRIVER = Driver()
	DRIVER.new_target(100.0, 0.0, True)
	rospy.spin()
	"""
	try:
