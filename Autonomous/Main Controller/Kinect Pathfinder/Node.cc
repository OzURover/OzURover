...
Node::Node (ORB_SLAM2::System::eSensor sensor, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) {
  ...
  status_publisher_ = node_handle_.advertise<geometry_msgs::PointStamped> (name_of_node_+"/status", 1);
...
void Node::Update () {
  ...
  PublishStatusAsPointStamped (orb_slam_->GetTrackingState());
...
void Node::PublishStatusAsPointStamped (int status) {
  geometry_msgs::PointStamped status_msg;
  status_msg.point.x = status;
