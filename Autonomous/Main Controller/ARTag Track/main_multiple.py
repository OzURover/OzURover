...
def fine_tune(marker):
	...
	dist = math.sqrt(97.5**2 + dist**2 - 2*97.5*dist*math.cos(math.radians(90-abs(vang))))
	print("Fine read: " + str(dist) + " cm / " + str(hang) + " deg")
...
def get_position():
	...
	print("Found markers: \n" + str(markers))
	return calculate_position(landmarks, markers, prev_position)
...
if __name__ == "__main__":
	...
	try:
		...
		while not rospy.core.is_shutdown():
			...
			DISTANCE, ANGLE = calculate_route(ref, waypoint, (x, y), landmarks)
			...
			if (x, y, ref) == (-1, -1, -1):
				print(
					"No ARTag was found. Cannot validate position and assuming encoder data was correct.")
				current_index = current_index + 1
			else:
				while True:
					print("Validating if correct location was reached...")
					error_distance = math.sqrt(
						math.pow(x - waypoint[1], 2) + math.pow(y - waypoint[2], 2))
					if error_distance > tolerance_radius:
						DISTANCE, ANGLE = calculate_route(
							ref, waypoint, (x, y), landmarks)
						DRIVER.new_target(DISTANCE, ANGLE)
						wait()
					else:
						current_index = current_index + 1
					...
					(x, y, ref) = get_current_position()
					if (x, y, ref) == (-1, -1, -1):
						print("No ARTag was found but this very unusual so assuming encoder data was correct")
						break
