...
class AutonomyController:
    ...
    def __init__(self, target_point, current_point, current_angle):
        self.tolerance_angle = 4 # tank drive moduna geçmek için minimum 
	...
	def is_rover_reached_checkpoint(self, current_point , partial_target_point):
		return ( current_point.x >= checkpoint.x ) && ( current_point.y >= checkpoint.y )
	
	
    def is_target_angle_reached(self, angle_traveled):
		return (angle_traveled + 360 < self.target_angle + 360 + self.tolerance_angle and angle_traveled + 360 > self.target_angle + 360 - self.tolerance_angle)
