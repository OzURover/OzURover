...
from geometry_msg import PointStamped
...
######################################################
    ...
    #####################################################
        ...
        self.timeout_ticks = rospy.get_param('~timeout_ticks',2000)
        self.ticks_per_meter = rospy.get_param('ticks_meter', 82091)
        self.vel_threshold = rospy.get_param('~vel_threshold', 0.000001) ## editable
        self.encoder_min = rospy.get_param('encoder_min', -2147483648)
        self.encoder_max = rospy.get_param('encoder_max', 2147483647)
        self.encoder_low_wrap = rospy.get_param('wheel_low_wrap', (self.encoder_max - self.encoder_min) * 0.3 + self.encoder_min ) ## editable
        self.encoder_high_wrap = rospy.get_param('wheel_high_wrap', (self.encoder_max - self.encoder_min) * 0.7 + self.encoder_min ) ## editable
        ...
        rospy.Subscriber("LM_EncoderTicks", Int32, self.wheelCallback)
        rospy.Subscriber("lm_wheel_velocity_target", Float32, self.targetCallback)
        self.pub_motor = rospy.Publisher('lm_motor_cmd', PointStamped, queue_size=10)
    ...
    #####################################################
        ...
        while not rospy.is_shutdown() and self.ticks_since_target < self.timeout_ticks:
            ...
            point_stamped_msg.point.x = self.motor
            self.pub_motor.publish(point_stamped_msg)
            self.r.sleep()
            self.ticks_since_target += 1
            if self.ticks_since_target == self.timeout_ticks:
                point_stamped_msg = PointStamped()
                point_stamped_msg.point.x = 0
                self.pub_motor.publish(point_stamped_msg)
