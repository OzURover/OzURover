...
PID armPID (&angle1, &PID_PWM, &GoalPosition,3500.0,80.0,30.0, DIRECT); //angle1 is encoder reading
...
void messageCb2(const std_msgs::Float32& curr_pos_j2){
  angle2 = curr_pos_j2.data ;
}
...
ros::Subscriber<std_msgs::Float32> sub2("curr_pos_j2", &messageCb2);
...
*/
  ...
  gravityTork = 1/2*(m1*g)*l1*cos(angle1*57.2958) + 1/2*(m2*g)*l2*cos(angle1*57.2958-angle2*57.2958) + m2*g*l1*cos(angle1*57.2958);
  gravityPWM = (gravityTork - 1.0132)/0.8554;
  
  if(PID_PWM > 0) CW(abs(PID_PWM)+gravityPWM);
  else if(PID_PWM < 0) CCW(abs(PID_PWM)-gravityPWM);
  else halt();
