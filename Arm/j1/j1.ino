...
//Motor constants
const float gear_ratio = 156.0;
const float ppt = 500.0; // Pulses per Turn
const float extra_constants = 7.5 * 1.27; //Extra coefficents (7.5 * 1.27(?))
const float Kp = 1000;
const float Kd = 0;
...
void loop() {
  ...
  if (abs(Err) <= 0.1) {
    ...
    /*Serial.print(meaAngle);
    Serial.print(" ");
    Serial.print(refAngle);
    Serial.print(" ");
    Serial.println(motVol);*/
  }
  setVolts(12);
}
...
float getDegree() {
  ...
  if (newPosition != oldPosition) {
    ...
    Serial.println(newPosition* (360.0 / (gear_ratio * ppt * extra_constants)));
